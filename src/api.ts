...
const instance = axios.create({
    baseURL: process.env.NODE_ENV === "development" ? "http://127.0.0.1:8000/api/v1/" : "https://airbnbclone-za96.onrender.com/api/v1",
...
export const checkBooking = ({ queryKey }: QueryFunctionContext<CheckBookingQueryKey> ) => {
    ...
    if(dates){
        ...
        const checkIn =  formatDate(firstDate); // const checkIn = firstDate.toLocaleDateString().replaceAll(". ", "-").replace(".","");
        const checkOut =  formatDate(secondDate); //const [checkOut] = secondDate.toJSON().split("T")
        return instance
